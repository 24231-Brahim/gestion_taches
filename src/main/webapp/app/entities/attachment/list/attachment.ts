...
import { AttachmentDeleteDialog } from '../delete/attachment-delete-dialog';
...
export class Attachment implements OnInit {
  ...
  readonly isLoading = this.attachmentService.attachmentsResource.isLoading;
  ...
  constructor() {
    effect(() => {
      const headers = this.attachmentService.attachmentsResource.headers();
      if (headers) {
    ...
    effect(() => {
      this.attachments.update(attachments =>
        this.fillComponentAttributesFromResponseBody([...this.attachmentService.attachments()], attachments),
      );
    });
  }
  ...
  delete(attachment: IAttachment): void {
    const modalRef = this.modalService.open(AttachmentDeleteDialog, { size: 'lg', backdrop: 'static' });
  ...
  protected fillComponentAttributesFromResponseBody(data: IAttachment[], currentValue: IAttachment[]): IAttachment[] {
    const attachmentsNew = [...currentValue];
    for (const d of data) {
      if (!attachmentsNew.some(op => op.id === d.id)) {
  ...
  protected queryBackend(): void {
    ...
    this.attachmentService.attachmentsParams.set(queryObject);
  }
