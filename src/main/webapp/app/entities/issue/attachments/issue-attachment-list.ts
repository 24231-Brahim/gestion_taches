...
@Component({
  ...
  selector: 'jhi-issue-attachment-list',
  templateUrl: './issue-attachment-list.html',
  styles: [
    `
      .issue-attachments {
        display: flex;
...
export class IssueAttachmentList implements OnInit {
  ...
  loadAttachments(): void {
    this.http.get<IAttachment[]>(`/api/attachments/by-issue/${this.issueId()}`).subscribe({
  ...
  uploadAttachment(file: File): void {
    ...
    this.http.post(`/api/attachments/upload?issueId=${this.issueId()}`, formData).subscribe({
