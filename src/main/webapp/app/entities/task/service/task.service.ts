...
type RestOf<T extends ITask | NewTask> = Omit<T, 'createdAt' | 'updatedAt'> & {
...
export class TaskService extends TasksService {
  ...
  assign(taskId: number, userId: number): Observable<ITask> {
    return this.http
      .patch<RestTask>(`${this.resourceUrl}/${taskId}/assign`, { userId })
      .pipe(map(res => this.convertResponseFromServer(res)));
  }
