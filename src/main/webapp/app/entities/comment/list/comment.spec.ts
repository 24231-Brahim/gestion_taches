...
describe('Comment Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 25492 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
  ...
  describe('trackId', () => {
    it('should forward to commentService', () => {
      ...
      vitest.spyOn(service, 'getCommentIdentifier');
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.commentsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
  ...
  it('should infinite scroll', async () => {
    ...
    req.flush([{ id: 25492 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
    expect(comp.comments().length).toEqual(1);
    expect(comp.comments()[0]).toEqual(expect.objectContaining({ id: 25492 }));

    // WHEN
    comp.loadNextPage();
    TestBed.tick();
    expect(service.commentsParams()).toMatchObject(expect.objectContaining({ page: '1' }));
    ...
    await vitest.runAllTimersAsync();
    expect(comp.comments().length).toEqual(2);
    expect(comp.comments()[1]).toEqual(expect.objectContaining({ id: 24616 }));

    comp.loadNextPage();
    TestBed.tick();
    expect(service.commentsParams()).toMatchObject(expect.objectContaining({ page: '2' }));
    req = httpMock.expectOne({ method: 'GET' });
    req.flush([{ id: 24616 }], {
      headers: { link: '<http://localhost/api/foo?page=0&size=20>; rel="prev",<http://localhost/api/foo?page=2&size=20>; rel="next"' },
    });
