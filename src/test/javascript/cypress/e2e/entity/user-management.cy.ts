...
describe('UserManagement e2e test', () => {
  ...
  let username: string;
  let password: string;
  const userManagementSample = { login: 'Cyrille_Guyot96', email: 'Melodie65@yahoo.fr' };
  ...
  describe('new UserManagement page', () => {
    ...
    it('should create an instance of UserManagement', () => {
      cy.get(`[data-cy="login"]`).type('Rejeanne.Pons31');
      cy.get(`[data-cy="login"]`).should('have.value', 'Rejeanne.Pons31');

      cy.get(`[data-cy="firstName"]`).type('Yvonne');
      cy.get(`[data-cy="firstName"]`).should('have.value', 'Yvonne');

      cy.get(`[data-cy="lastName"]`).type('Rousseau');
      cy.get(`[data-cy="lastName"]`).should('have.value', 'Rousseau');

      cy.get(`[data-cy="email"]`).type('Waleran_Moreau92@yahoo.fr');
      cy.get(`[data-cy="email"]`).should('have.value', 'Waleran_Moreau92@yahoo.fr');
