...
| 6   | `./images/capture-detail-task.png`   | Détail d'une tâche avec commentaires         |
...
L'application **Gestion de Tâches** est une plateforme web complète de gestion de projets, calquée sur le modèle de Jira. Elle permet à des équipes de planifier, suivre et organiser leur travail à travers des sprints, des epics et des tâches.
...
- Permettre aux utilisateurs de créer et suivre des tâches
...
| ------------------ | -------------------------------------------------- | --------------------------------------------------- |
| **Project**        | Projet racine                                      | owner (User), projectMembers, sprints, epics, tasks |
...
| **Task**           | Unité de travail                                   | Appartient à un Project, optionnel : Sprint, Epic   |
| **TaskComment**    | Commentaire sur une tâche                          | Lié à une Task                                      |
| **TaskAttachment** | Fichier joint à une tâche                          | Lié à une Task                                      |
| **TaskHistory**    | Audit des modifications                            | Lié à une Task                                      |
...
└─────────────┘       │ createdAt    │
       ...
       │ id        │ │ id       │ │ id           │
       │ name      │ │ title    │ │ title         │
       │ goal      │ │ status   │ │ status (enum) │
       │ startDate │ │ priority │ │ priority      │
       │ endDate   │ │ createdAt│ │ createdAt     │
       │ status    │ │ project  │ │ updatedAt     │
       │ project   │ └──────────┘ │ sprint (opt.) │
       └───────────┘              │ epic (opt.)   │
                                  │ project       │
                                  │ commentses    │──► Set<TaskComment>
                                  │ attachmentses │──► Set<TaskAttachment>
                         ...
                         │                 │                  │                     │
                ┌────────▼────┐  ┌─────────▼──────┐  ┌───────▼───────────┐  ┌──────▼──────────┐
                │TaskComment  │  │TaskAttachment   │  │   TaskHistory     │  │  Notification   │
                │────────────│  │───────────────│  │───────────────────│  │─────────────────│
                │ id         │  │ id            │  │ id                │  │ id              │
                │ content    │  │ fileName      │  │ action            │  │ userId          │
                │ createdAt  │  │ filePath      │  │ fieldChanged      │  │ message         │
                │ task       │  │ uploadedAt    │  │ oldValue          │  │ taskId          │
                └────────────┘  │ task          │  │ newValue          │  │ taskTitle       │
                                └───────────────┘  │ createdAt         │  │ isRead          │
                                                   │ task              │  │ createdAt       │
                                                   └───────────────────┘  └─────────────────┘
```
...
### 5.2 Gestion des Sprints, Epics et Tâches

- CRUD complet pour chaque entité
- Filtrage et pagination avancée via des _criteria_ (TaskQueryService, SprintQueryService, EpicQueryService)
- Statuts : NEW, TODO, IN_PROGRESS, IN_REVIEW, DONE, CANCELLED (Task) ; PLANNED, ACTIVE, COMPLETED, CANCELLED (Sprint)
- Priorités : LOWEST, LOW, MEDIUM, HIGH, HIGHEST
- Association d'une tâche à un sprint et/ou un epic
...
**Refactoring Task (Backlog/Board + Detail Drawer) :**

- Page task list refondue avec deux onglets : Backlog (table paginée avec recherche textuelle et filtres) et Board (Kanban avec drag-and-drop par colonne TaskStatus)
- TaskDetailPanel : drawer latéral avec statut (select), description éditable, informations détaillées (sprint, epic, priority, dates, project), et sections intégrées pour commentaires, pièces jointes et historique

### 5.3 Commentaires, Attachements et Historique

- **Commentaires** : création, modification, suppression sur les tâches. Ajout du champ `author` (User). Nouvel endpoint `GET /api/task-comments/by-task/{taskId}`. Auto-set de l'auteur à la création via `SecurityUtils.getCurrentUserLogin()`.
- **Attachements** : upload réel de fichiers via `POST /api/task-attachments/upload?taskId=` avec stockage disque configurable (`${app.upload.dir:uploads}`). Download via `GET /api/task-attachments/download/{id}`. Nouvel endpoint `GET /api/task-attachments/by-task/{taskId}`.
- **TaskHistory** : audit trail complet des modifications sur une tâche (champ modifié, ancienne/nouvelle valeur). Nouvel endpoint `GET /api/task-histories/by-task/{taskId}`.
...
| Assigner une Task               | ✅    | ✅             | ✅        | ❌   |
| TaskComment (POST)              | ✅    | ✅             | ✅        | ✅   |
| TaskComment (PUT/PATCH/DEL)     | ✅    | ✅             | ✅        | ❌   |
| TaskAttachment/TaskHistory CRUD | ✅    | ✅             | ✅        | ❌   |
| GET (lecture)                   | ✅    | ✅             | ✅        | ✅   |
...
- **BottomNav** mobile (visible <768px, 4 items : Accueil, Projets, Tâches, Settings/Login)
...
- **TaskDetailPanel** : drawer latéral pour la consultation et modification rapide d'une tâche
- **TaskKanbanBoard** : drag-and-drop natif par colonne de statut
...
- **Gestion des erreurs** avec AlertService sur tous les composants CRUD (Project, Sprint, Epic, Task, TaskComment, TaskAttachment, TaskHistory)
...
Depuis un commentaire n'appartient qu'à son auteur, seul l'utilisateur ayant créé le commentaire ou un administrateur (PROJET_MANAGER / ADMIN) peut le modifier ou le supprimer. Une méthode utilitaire `checkCommentOwnership()` dans `TaskCommentResource.java` vérifie que l'utilisateur courant est bien l'auteur du commentaire avant toute opération PUT/PATCH/DELETE ; si ce n'est pas le cas, une `BadRequestAlertException("Access denied")` est levée.

### 5.9 Assignation des Tâches

L'assignation d'une tâche à un utilisateur est une fonctionnalité propre au workflow de gestion de projet. Elle permet d'attribuer une tâche à un développeur ou à un chef de projet.

**Côté backend :**

- Endpoint `PATCH /api/tasks/{id}/assign` dans `TaskResource.java` — accepte un body contenant l'ID de l'utilisateur assigné
- Seuls les rôles DEVELOPER, PROJET_MANAGER et ADMIN peuvent assigner une tâche
- L'assignee est stocké via une relation `@ManyToOne` entre `Task` et `User` (colonne `assignee_id`)
- `TaskService.assign()` met à jour l'assignee et sauvegarde l'entité
...
- `TaskService.assign()` et `getAssignableUsers()` dans `task.service.ts`
- Un `<select>` déroulant dans le drawer de détail (`TaskDetailPanel`) permet de choisir l'assignee
...
Un système de notification in-app a été mis en place pour informer les utilisateurs lorsqu'une tâche leur est assignée.

**Côté backend :**

- Nouvelle entité `Notification.java` avec les champs : `id`, `userId` (destinataire), `message`, `taskId`, `taskTitle`, `isRead` (booléen), `createdAt`
...
- Le message généré est de la forme : _"\[username\] vous a assigné à la tâche #{id} : {title}"_
...
- Au clic sur une notification : marquage comme lu + navigation vers la tâche concernée
...
- **TaskHistory** : audit trail complet des modifications sur les tâches
...
- **Graphiques** : diagramme de progression (barres horizontales par projet) et donut (répartition des statuts de tâches) en SVG pur
...
- **Quick Actions** : boutons d'accès rapide (Nouveau Projet, Nouvelle Tâche, Voir Board, Voir Roadmap)
- **Gestion des erreurs** : bannières de chargement et d'erreur sur chaque section
- Données de seed démo (2 projets, 4 sprints, 4 epics, 10 tâches, 5 commentaires, 6 membres, 7 actions)
...
| `![Détail d'une tâche](./images/capture-detail-task.png)`  | Vue détaillée d'une tâche avec commentaires, historique et pièces jointes  |
...
- [x] **Gestion des erreurs frontend unifiée** : appliquer le fix `onSaveError()` avec `AlertService` sur tous les composants (Sprint, Epic, Task, TaskComment, TaskAttachment, TaskHistory)
...
- [ ] **Tests complets des nouvelles fonctionnalités** : ajouter des tests d'intégration pour l'assignation des tâches et les notifications
...
- [ ] **Tableau Kanban complet** : intégration du drag-and-drop du statut des tâches sur le dashboard
- [ ] **Page de liste des notifications** : route dédiée avec historique complet
- [ ] **Notifications par email** : envoi d'un email lors de l'assignation d'une tâche
...
| CRUD Sprint/Epic/Task                       | ✅ Terminé | Avec critères de recherche         |
| CRUD TaskComment/TaskAttachment/TaskHistory | ✅ Terminé | Audit trail fonctionnel            |
...
| Refactoring Task (Backlog/Board + Drawer)    | ✅ Terminé | Onglets, Kanban, TaskDetailPanel               |
| TaskComment author + endpoints by-task       | ✅ Terminé | Endpoints dédiés Comment/Attachment/History    |
| Upload/download fichiers (TaskAttachment)    | ✅ Terminé | Multipart, stockage disque, download           |
| Ownership des commentaires                   | ✅ Terminé | Vérification auteur avant PUT/PATCH/DELETE     |
| Assignation des tâches                       | ✅ Terminé | PATCH + assignable users + UI drawer           |
...
Les trois fonctionnalités ajoutées lors de cette phase — règles métier sur les commentaires, assignation des tâches et notifications — complètent le socle fonctionnel de l'application en répondant à des besoins concrets de workflow agile. L'assignation avec notification automatique crée un circuit de communication entre les membres de l'équipe, tandis que la protection des commentaires renforce la fiabilité des données.
